...
{	
	...
	if ($count_matches != 0) {
		...
		for($i = 0; $i < $count_matches; $i++) {
			...
			if ($tmpl['detailwindow']  == 9 || $tmpl['detailwindow']  == 10) {
				$tmpl['detailwindow'] = 2;
			}
			...
			if ($tmpl['enablecustomcss'] == 1) {
				JHTML::stylesheet('components/com_phocagallery/assets/phocagallerycustom.css' );
				PhocaGalleryRenderFront::displayCustomCSS($tmpl['customcss']);
			}
				...
				if ($img_cat == 1) {
					...
					foreach ($data_outcome_array as $category) {
						...
						if (isset($category)) {
							...
							$rightDisplayKey = PhocaGalleryAccess::getUserRight('accessuserid', $category->accessuserid, $category->access, $user->authorisedLevels(), $user->get('id', 0), 0);
						}
							...
							if ($display_icon_detail == 1 || $display_icon_download > 0 || $enable_piclens == 2) {
								...
								if ($display_icon_download > 0) {
									...
									if ((int)$display_icon_download == 2) {
										$output .= ' <a title="'. JText::_('PLG_CONTENT_PHOCAGALLERY_IMAGE_DOWNLOAD').'" href="'. JRoute::_($image->linkother . '&amp;phocadownload='.(int)$display_icon_download).'"';
									} else {
										$output .= ' <a class="'. $button->methodname.'" title="'. JText::_('PLG_CONTENT_PHOCAGALLERY_IMAGE_DOWNLOAD').'" href="'. JRoute::_($image->linkother . '&amp;phocadownload='.(int)$display_icon_download).'"';
