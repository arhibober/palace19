...
class xmap_com_k2 
{
	...
	function getTree( &$xmap, &$parent, &$params ) 
	{
		...
		if ($xmap->view == "xml")
			...
			self::$maxAccess = implode(",", JFactory::getUser()->authorisedLevels());      
		switch(xmap_com_k2::getParam($link_vars,'task',""))
		{
			case "user":
				$ids[0] = xmap_com_k2::getParam($link_vars, 'id', $id);	
				$mode = "single user";
				break;
			case "tag":
				$tag = xmap_com_k2::getParam($link_vars, 'tag',"");		
				$ids = $parm_vars['categoriesFilter'];
				$mode = "tag";
				break;
			case "category":
			...
			case "":
				switch(xmap_com_k2::getParam($link_vars,'layout',""))
				{
					...
					case "latest":
						...
						{
							$ids = $parm_vars["userIDs"];	
							$mode = "latest user";
						}
						else
						{
							$ids = $parm_vars["categoryIDs"];
	...
	function processTree($db, &$xmap, &$parent, &$params, $mode, $ids, $tag, $limit)
	{
	...
	function addNode($xmap, $node, $row, $iscat, &$parent, &$params)
	{
