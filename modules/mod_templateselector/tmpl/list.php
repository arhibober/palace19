...
$duration = intval($params->get('duration', 365));
...
<?php foreach($lists as $k =>$row):?>
	...
	<li class="tplselectoritem<?php echo ($k%2? ' odd':'');?><?php
		...
		<?php if(!$row->current):?>
		<a href="javascript:void(0);" onclick="Cookie.write('jTemplateSelector',<?php
			echo $row->value;?>,{duration:<?php
			echo $duration;?>});window.location.reload();" title="<?php
			echo $title;?>">
		<?php endif;?>
			<?php if($displayThumb):?>
